...
const appVersion = '0.6.1.2.pre.4';
...
async function handle(req,res) {
  ...
  if(u.pathname==='/api/fonts/apply'&&req.method==='POST') { try { const {files,fields}=await readMultipartRequest(req); const operations=JSON.parse(fields.operations||'[]'); for(const {name,data} of files){try{await fsp.writeFile(path.join(fontsDir,name),data,{flag:'wx',mode:0o644});}catch(e){if(e.code!=='EEXIST')throw e;}} for(const op of operations){const name=safeFontName(op.name);if(!name)continue;const active=path.join(fontsDir,name),disabled=path.join(disabledFontsDir,name);if(op.action==='disable'){await fsp.rename(active,disabled).catch(e=>{if(e.code!=='ENOENT')throw e;});}else if(op.action==='enable'){await fsp.rename(disabled,active).catch(e=>{if(e.code!=='ENOENT')throw e;});}else if(op.action==='delete'){await fsp.rm(active,{force:true});await fsp.rm(disabled,{force:true});}} await syncFontsToContainer(); await restartOnlyOffice(); return json(res,200,{ok:true,fonts:await listFonts()}); } catch(e) { return json(res,400,{error:'font_apply_failed',message:e.message}); } }
