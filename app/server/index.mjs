...
const appVersion = '0.6.1.2.pre.21';
...
let fontApplyState={busy:false,startedAt:0};
...
async function readMultipartRequest(req){const type=String(req.headers['content-type']||'');const match=type.match(/boundary=(?:"([^"]+)"|([^;]+))/i);if(!match)throw new Error('multipart_boundary_missing');const boundary=Buffer.from(`--${match[1]||match[2]}`);const body=await readBody(req);const files=[],fields={};let cursor=0;while(cursor<body.length){const start=body.indexOf(boundary,cursor);if(start<0)break;const headerStart=start+boundary.length;if(body.subarray(headerStart,headerStart+2).toString()==='--')break;const contentStart=body.indexOf(Buffer.from('\r\n\r\n'),headerStart);if(contentStart<0)break;let dataEnd=body.indexOf(boundary,contentStart+4);if(dataEnd<0)dataEnd=body.length;if(dataEnd>=2&&body[dataEnd-2]===13&&body[dataEnd-1]===10)dataEnd-=2;const disposition=body.subarray(headerStart,contentStart).toString();const fieldMatch=disposition.match(/name="([^"]+)"/i);const filename=disposition.match(/filename="([^"]*)"/i);const data=body.subarray(contentStart+4,dataEnd);if(filename){const name=safeFontName(filename[1]);if(name&&data.length)files.push({name,data});}else if(fieldMatch)fields[fieldMatch[1]]=data.toString('utf8');cursor=dataEnd;}return {files,fields};}
...
async function handle(req,res) {
  ...
  if ((u.pathname==='/fonts'||u.pathname==='/api/fonts'||u.pathname==='/api/fonts/upload'||u.pathname==='/api/fonts/apply'||u.pathname==='/api/fonts/action')&&!who.isAdmin) {
  ...
  if(u.pathname==='/api/fonts/status'&&req.method==='GET'){if(fontApplyState.busy){const age=Date.now()-fontApplyState.startedAt;const running=age>30000&&await dockerRun(['inspect','-f','{{.State.Running}}','fnoffice-onlyoffice']).then(v=>String(v).trim()==='true').catch(()=>false);if((running&&await probeOnlyOffice())||age>2*60*1000)fontApplyState={busy:false,startedAt:0}}return json(res,200,fontApplyState)}
  if(u.pathname==='/api/fonts/status'&&req.method==='POST'){try{const body=JSON.parse((await readBody(req)).toString()||'{}');fontApplyState=body.busy?{busy:true,startedAt:Date.now()}:{busy:false,startedAt:0};return json(res,200,fontApplyState)}catch(e){return json(res,400,{error:'font_status_invalid'})}}
  ...
  if(u.pathname==='/api/fonts/apply'&&req.method==='POST') { try { const {files,fields}=await readMultipartRequest(req); const operations=JSON.parse(fields.operations||'[]'); let uploadTargets={},overwrite=[]; try{uploadTargets=JSON.parse(fields.uploadTargets||'{}')}catch{}; try{overwrite=JSON.parse(fields.overwrite||'[]')}catch{}; const overwriteSet=new Set(overwrite.map(safeFontName)); const before=(await listFonts()).enabled.slice().sort(); await fsp.mkdir(fontsDir,{recursive:true}); await fsp.mkdir(disabledFontsDir,{recursive:true}); const conflicts=[]; for(const {name} of files){const target=uploadTargets[name]==='disabled'?disabledFontsDir:fontsDir;if(await fsp.stat(path.join(target,name)).then(()=>true).catch(()=>false)&&!overwriteSet.has(name))conflicts.push(name);} if(conflicts.length)return json(res,409,{error:'font_conflicts',conflicts}); for(const {name,data} of files){const target=uploadTargets[name]==='disabled'?disabledFontsDir:fontsDir;await fsp.writeFile(path.join(target,name),data,{flag:overwriteSet.has(name)?'w':'wx',mode:0o644});} for(const op of operations){const name=safeFontName(op.name);if(!name)continue;const active=path.join(fontsDir,name),disabled=path.join(disabledFontsDir,name);if(op.action==='disable'){await fsp.rename(active,disabled).catch(e=>{if(e.code!=='ENOENT')throw e;});}else if(op.action==='enable'){await fsp.rename(disabled,active).catch(e=>{if(e.code!=='ENOENT')throw e;});}else if(op.action==='delete'){await fsp.rm(active,{force:true});await fsp.rm(disabled,{force:true});}} const after=(await listFonts()).enabled.slice().sort(); const enabledChanged=JSON.stringify(before)!==JSON.stringify(after); if(enabledChanged){await syncFontsToContainer();await restartOnlyOffice();} return json(res,200,{ok:true,restarted:enabledChanged,fonts:await listFonts()}); } catch(e) { return json(res,400,{error:'font_apply_failed',message:e.message}); } }
...
const markFontApplyStart=()=>{fontApplyState={busy:true,startedAt:Date.now()}};
